...
class DisplayViewController: UIViewController {
    ...
    @IBOutlet weak var uberWasEnabled: UIButton!
    ...
    override func viewDidLoad() {
        ...
        if User.currentUser != nil {
            uberWasEnabled.isHidden = !(m_selectedEvent.eventUberEnabled)!
        } else {
            uberWasEnabled.isHidden = true
        }
    ...
    @IBAction func uberDeepLink(_ sender: UIButton) {
        let button = RideRequestButton()
        let dropoffLocation = GlobalService.sharedInstance.userLocation
        
        let builder = RideParametersBuilder().setDropoffLocation(dropoffLocation!, nickname: m_selectedEvent.eventAddress ?? "Your Event")
        button.rideParameters = builder.build()
        
        view.addSubview(button)
        button.frame.origin = CGPoint(x: 16, y: ScreenHeight - 50)
    }
